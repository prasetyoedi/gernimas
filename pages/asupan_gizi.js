...
const AsupanGiziTrimaster = [
    {
        ...
        description: "Trimester 1 dimulai dari konsepsi sampai 3 bulan (0-12 minggu)"
    },
    {
                ...
                <div className="flex flex-col w-full h-auto md:h-[200px] px-4 md:px-[35px] py-4 flex-shrink-0 rounded-[20px] bg-[rgba(255,140,157,0.20)] gap-4">
                    <div className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-[296px]">
                        <div className="flex flex-col w-full md:w-[720px] items-start gap-4">
                            ...
                            <h2 className="text-red-400 text-base md:text-lg font-medium leading-normal">Selama kehamilan jangan lupa mengonsumsi makanan yang bergizi dan dapat disesuaikan dengan trimester kehamilan, ya!</h2>
                        </div>
                        <img src="/assets/images/services/woman.png" className="w-[218px] h-auto md:h-[163px]" />
                    </div>
                </div>
                <div className="flex flex-row w-full mt-6 h-auto flex-shrink-0 rounded-[20px] gap-20">
                    {AsupanGiziTrimaster.map((card, index) => (
                        <AsupanGiziCard key={index} imageSrc={card.imageSrc} title={card.title} description={card.description} />
                    ))}
                </div>
            </div>
            <Footer />
